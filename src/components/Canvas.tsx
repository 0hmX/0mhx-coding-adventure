...
const Canvas: React.FC<CanvasProps> = ({ 
  ...
  // Effect for initial setup and grid rendering
  useEffect(() => {
  ...
  // Effect for executing Lua code
  useEffect(() => {
    if (!luaInterpreter || !luaCode.trim() || !canvasRef.current || !shouldRun) return;
  ...
  }, [width, height, gridSize, luaCode, luaInterpreter, shouldRun]);
  
  // Initial run on component mount if interpreter is available
  useEffect(() => {
    if (luaInterpreter && canvasRef.current) {
      // Force an initial run when the interpreter becomes available
      const canvas = canvasRef.current;
      const ctx = canvas.getContext('2d');
      if (!ctx) return;
  ...
  }, [luaInterpreter, width, height, gridSize, luaCode]);
